...
# Was ist eine Control Unit?: Inhalt übernommen! (Quelle): https://www.elektronik-kompendium.de/sites/com/1310171.htm, 22.01.2021)
...
# Kleines Vorwort vorweg:
#   Mein Programm zur Visualisierung einer Control-Unit besteht aus 2 großen Eiheiten:
#       Einer Einheit zur Ausführung und Berechung von den Befehlen
#       Einer Einheit zur Annahme von Befehlen durch periphere Geräte, und zur decodierung der Befehle
#   Die Überwachung des Systems findet durchgehend statt. Wenn zum Beispiel irgendwo eine falsche Eingabe stattfindet, dann wird der aktuelle Prozess abgebrochen und der Fehler wird ausgegeben.
...
# Prozess zum Lesen einer bestimmten Information aus dem Ram
def readCertainDataRam():
    if len(dataRam) != 0:
        ...
        try:
            ...
            return ("Keine gültige Eingabe! (Probieren Sie '1' etc.)")
    else:
        return ("Es sind noch keine Einträge zwischengespeichert worden!")
...
# Prozess zum Löschen von einen Eintrag aus dem Ram
def emptyOneRam():
    if len(dataRam) != 0:
        whichEntry = input("Welches Objekt möchten Sie entfernen?: ")
        try:
            dataRam.remove(whichEntry)
            return ("Es wurde " + whichEntry + " aus dem Ram gelöscht")
        except ValueError:
            return ("Der Eintrag '" + whichEntry + "' existiert nicht!")
    else:
        return ("Es sind noch keine Einträge zwischengespeichert worden!")

# Prozess zum Sortieren vom Ram
def sortRam():
    if len(dataRam) != 0 and len(dataRam) != 1:
        try:
            dataRam.sort()
            return ("Liste wurde sortiert...")
        except:
            return ("Liste konnte nicht sortiert werden!")
    elif len(dataRam) == 1:
        return ("Es ist nur ein Eintrag im Ram vorhanden!")
    else:
        return ("Es sind noch keine Einträge zwischengespeichert worden!")
...
while 1:
    ...
    if eingabe.isdigit():
        ...
        if varEingabe < 1 or varEingabe > 7:
